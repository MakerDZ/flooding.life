...
const calculateCanvasSize = (nodeCount: number) => {
    ...
    const windowWidth =
        typeof window !== 'undefined' ? window.innerWidth : 1920; // Default to 1920 if window is not available
    const windowHeight =
        typeof window !== 'undefined' ? window.innerHeight : 1080; // Default to 1080 if window is not available

    // Ensure the canvas is at least as large as the window
    const finalWidth = Math.max(
        Math.ceil(canvasWidth) + PADDING * 2,
        windowWidth
    );
    const finalHeight = Math.max(
        Math.ceil(canvasHeight) + PADDING * 2,
        windowHeight
    );
...
export const useNodeFlow = (data: any) => {
    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            const newSize = calculateCanvasSize(data.length);
